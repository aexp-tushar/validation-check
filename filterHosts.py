...
ngx = False
cdapi = False
for cdhost in hosts['CDaaS'].keys():
    if hosts['CDaaS'][cdhost][2] == "NGINX Content Server":
        ngx = True
    if hosts['CDaaS'][cdhost][2] == "CDAAS API":
        cdapi = True
for service in hosts.keys():
    if len(hosts[service]) != 0:
        path = "host_"+service
